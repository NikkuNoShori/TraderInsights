...
const API_KEY = import.meta.env.VITE_ALPHA_VANTAGE_API_KEY || '2WZT5WFVQM1I5R14';
...
api.interceptors.response.use(
  ...
  error => {
    if (axios.isAxiosError(error)) {
      ...
      throw new Error(error.response.data?.message || 'Failed to fetch data from API.');
    }
    throw error;
  }
);
...
export async function fetchStockQuote(symbol: string): Promise<StockQuote> {
  ...
  try {
    ...
    return {
      ...
      avgVolume3Month: Number(overviewResponse.data['3MonthAverageVolume']) || 0,
    ...
    if (error instanceof Error) {
      throw new Error(`Failed to fetch quote for ${validSymbol}: ${error.message}`);
    }
    throw error;
  }
}