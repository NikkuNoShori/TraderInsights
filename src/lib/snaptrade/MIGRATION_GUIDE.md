...
const { redirectURI } = await snapTradeService.createConnectionLink('brokerageId');
...
await snapTradeService.init({
  ...
  // No need to specify redirectUri for testing
});
...
1. Make sure your broker callback component correctly handles the parameters returned by SnapTrade.
2. Verify that your application is properly registered with SnapTrade.
3. Check that the callback route is accessible and properly configured in your routing setup.
