...
const ErrorMonitoring: React.FC = () => {
  const [errorStats, setErrorStats] = useState<ErrorStats>({ low: 0, medium: 0, high: 0, critical: 0 });
          ...
          <TableBody>
            {recentErrors.map((error) => (
              <TableRow key={error.id}>
                ...
                <TableCell>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium bg-${severityColors[error.severity]}-100 text-${severityColors[error.severity]}-800`}>
