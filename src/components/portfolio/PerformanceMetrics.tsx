...
function MetricCard({ title, value, icon: Icon, trend = 'neutral', subtitle }: MetricCardProps) {
  return (
    <div className="card p-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          {subtitle && (
            <p className="mt-1 text-sm text-gray-500 dark:text-dark-muted">{subtitle}</p>
          )}
        </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          icon={Percent}
          trend={metrics.profitFactor > 1 ? 'up' : metrics.profitFactor < 1 ? 'down' : 'neutral'}
        />
      </div>
